...
export function OverviewTab({ data }: OverviewTabProps) {
  ...
  const avgSupport = supportStats.reduce((sum, s) => sum + s.mean, 0) / 6;
  const avgChallenge = challengeStats.reduce((sum, s) => sum + s.mean, 0) / 6;
            ...
            {supportStats.map((stat, idx) => (
              <div key={idx} className="border-b border-border pb-2 last:border-0">
                ...
                <div className="flex justify-between text-xs text-muted-foreground">
                  <span>Median: {stat.median.toFixed(1)}</span>
                  <span>{stat.percentHighUse.toFixed(0)}% use frequently (4-5)</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <Card className="p-6">
        ...
        <div className="space-y-2 text-sm">
          <p className="text-muted-foreground">
            <strong>Most frequent support strategy:</strong> {supportStats.reduce((max, s) => s.mean > max.mean ? s : max).question} (avg: {supportStats.reduce((max, s) => s.mean > max.mean ? s : max).mean.toFixed(2)})
          </p>
          <p className="text-muted-foreground">
            <strong>Most frequent challenge strategy:</strong> {challengeStats.reduce((max, s) => s.mean > max.mean ? s : max).question} (avg: {challengeStats.reduce((max, s) => s.mean > max.mean ? s : max).mean.toFixed(2)})
          </p>
          <p className="text-muted-foreground">
